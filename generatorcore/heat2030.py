...
def Heat2030_calc(root):
    ...
    try:
        ...
        p_heatnet.energy = (
            ...
            i30.s_renew_heatnet.energy
        )
        g_storage.energy = p_heatnet.energy
        g_storage.pct_energy = (
            fact('Fact_H_P_storage_specific_volume')
        )
        g_storage.power_to_be_installed = (
            g_storage.energy *
            g_storage.pct_energy
        )
        p_heatnet_plant.invest_per_x = (
            fact('Fact_H_P_heatnet_solarth_park_invest_203X')
        )
        g_storage.invest = (
            g_storage.invest_per_x *
            g_storage.power_to_be_installed
        )
        p_heatnet_lheatpump.invest_per_x = (
            fact('Fact_H_P_heatnet_lheatpump_invest_203X')
        )
        g_storage.invest_com = g_storage.invest

        p_heatnet_plant.pct_energy = (
            ass('Ass_H_P_heatnet_fraction_solarth_2050')
        )
        p_heatnet_cogen.energy = (
            e30.p_local_biomass_cogen.energy
            if (e30.p_local_biomass_cogen.energy < p_heatnet.energy) else
            p_heatnet.energy
        )
        p_heatnet_plant.energy = (
            (p_heatnet.energy - p_heatnet_cogen.energy) * p_heatnet_plant.pct_energy
            if (p_heatnet_cogen.energy < p_heatnet.energy) else
            0
        )
        g_planning.invest_com = g_planning.invest

        g_storage.invest_pa = (
            g_storage.invest / entry('In_M_duration_target')
        )
        g_planning.invest_pa_com = g_planning.invest_pa

        g_storage.invest_pa_com = g_storage.invest_pa

        g_storage.pct_of_wage = (
            fact('Fact_B_P_constr_main_revenue_pct_of_wage_2017')
        )
        g.invest_com = (
            g_storage.invest_com +
            g_planning.invest_com
        )
        g_storage.ratio_wage_to_emplo = (
            fact('Fact_B_P_constr_main_ratio_wage_to_emplo_2017')
        )
        g_planning.cost_wage = (
            g_planning.invest / fact('Fact_H_P_planning_duration')
        )
        g_storage.cost_wage = (
            g_storage.pct_of_wage *
            g_storage.invest_pa
        )
        g_planning.ratio_wage_to_emplo = (
            ass('Ass_T_C_yearly_costs_per_planer')
        )
        g_storage.demand_emplo = (
            g_storage.cost_wage / g_storage.ratio_wage_to_emplo
        )
        g_planning.demand_emplo = (
            g_planning.cost_wage / g_planning.ratio_wage_to_emplo
        )
        g.invest_pa_com = (
            g_storage.invest_pa_com +
            g_planning.invest_pa_com
        )
        ...
        p_heatnet_plant.area_ha_available = (
            p_heatnet_plant.energy / fact('Fact_H_P_heatnet_solarth_park_yield_2025')
        )
        g.invest = (
            g_storage.invest +
            g_planning.invest
        ...
        g_planning.pct_of_wage = (
            ass('Ass_H_G_planning_cost_pct_of_wage')
        )
        g.cost_wage = (
            g_storage.cost_wage +
            g_planning.cost_wage
        ...
        p_heatnet_lheatpump.pct_energy = (
            ass('Ass_H_P_heatnet_fraction_lheatpump_2050')
        )
        p_fueloil.energy = (
            r30.s_fueloil.energy +
            b30.s_fueloil.energy
        )
        p_opetpro.energy = 0
        p_gas.cost_fuel_per_MWh = (
            ass('Ass_R_S_gas_energy_cost_factor_2035')
        )
        ...
        p_fueloil.CO2e_cb_per_MWh = (
            h18.p_fueloil.CO2e_cb_per_MWh
        )
        p_fueloil.CO2e_cb = (
            p_fueloil.CO2e_cb_per_MWh *
            p_fueloil.energy
        ...
        p_heatnet_lheatpump.full_load_hour = (
            fact('Fact_H_P_heatnet_lheatpump_full_load_hours')
        )
        ...
        p_heatnet_geoth.invest_per_x = (
            fact('Fact_H_P_heatnet_geoth_invest_203X')
        )
        p_heatnet_geoth.full_load_hour = (
            fact('Fact_H_P_heatnet_geoth_full_load_hours')
        )
        p_heatnet_lheatpump.pct_of_wage = (
            fact('Fact_B_P_constr_main_revenue_pct_of_wage_2017')
        )
        p_heatnet_lheatpump.power_to_be_installed = (
            p_heatnet_lheatpump.energy / p_heatnet_lheatpump.full_load_hour
        )
        p_heatnet_plant.invest_pa = (
            p_heatnet_plant.invest / entry('In_M_duration_target')
        )
        ...
        p_fueloil.cost_fuel_per_MWh = (
            ass('Ass_R_S_fueloil_energy_cost_factor_2035')
        )
        p_gas.cost_fuel = (
            p_gas.energy *
            p_gas.cost_fuel_per_MWh / Million
        )
        p_opetpro.CO2e_pb = (
            p_opetpro.energy *
            p_opetpro.CO2e_pb_per_MWh
        )
        p_opetpro.CO2e_cb_per_MWh = (
            h18.p_opetpro.CO2e_cb_per_MWh
        )
        p_opetpro.CO2e_cb = (
            p_opetpro.CO2e_cb_per_MWh *
            p_opetpro.energy
        ...
        p_coal.CO2e_pb_per_MWh = (
            h18.p_coal.CO2e_pb_per_MWh
        )
        p_gas.change_energy_MWh = (
            p_gas.energy -
            h18.p_gas.energy
        )
        p_gas.change_energy_pct = (
            p_gas.change_energy_MWh / h18.p_gas.energy
        )
        p_gas.change_CO2e_t = (
            p_gas.CO2e_total -
            h18.p_gas.CO2e_total
        )
        p_gas.change_CO2e_pct = (
            p_gas.change_CO2e_t / h18.p_gas.CO2e_total
        )
        p_gas.CO2e_total_2021_estimated = (
            h18.p_gas.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_ofossil.energy = 0
        p_coal.CO2e_pb = (
            p_coal.energy * p_coal.CO2e_pb_per_MWh
        )
        p_coal.CO2e_cb_per_MWh = h18.p_coal.CO2e_cb_per_MWh

        p_coal.CO2e_cb = (
            p_coal.CO2e_cb_per_MWh * p_coal.energy
        )
        p_lpg.change_energy_MWh = (
            p_lpg.energy - h18.p_lpg.energy
        )
        p_lpg.change_energy_pct = (
            p_lpg.change_energy_MWh / h18.p_lpg.energy
        )
        p_lpg.change_CO2e_t = (
            p_lpg.CO2e_total - h18.p_lpg.CO2e_total
        )
        p_lpg.change_CO2e_pct = (
            p_lpg.change_CO2e_t / h18.p_lpg.CO2e_total
        )
        p_lpg.CO2e_total_2021_estimated = (
            h18.p_lpg.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_opetpro.pct_energy = (
            p_opetpro.energy / p.energy
        )
        p_coal.cost_fuel_per_MWh = (
            ass('Ass_R_S_coal_energy_cost_factor_2035')
        )
        p_fueloil.cost_fuel = (
            p_fueloil.energy *
            p_fueloil.cost_fuel_per_MWh / Million
        )
        p_coal.CO2e_total = (
            p_coal.CO2e_pb +
            p_coal.CO2e_cb
        )
        p_heatnet_cogen.CO2e_pb_per_MWh = (
        ...
        p_fueloil.change_energy_MWh = (
            p_fueloil.energy -
            h18.p_fueloil.energy
        )
        p_fueloil.change_energy_pct = (
            p_fueloil.change_energy_MWh / h18.p_fueloil.energy
        )
        p_fueloil.change_CO2e_t = (
            p_fueloil.CO2e_total - h18.p_fueloil.CO2e_total
        )
        p_fueloil.change_CO2e_pct = (
            p_fueloil.change_CO2e_t / h18.p_fueloil.CO2e_total
        )
        p_fueloil.CO2e_total_2021_estimated = (
            h18.p_fueloil.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_lpg.pct_energy = (
            p_lpg.energy / p.energy
        )
        p_coal.pct_energy = (
            p_coal.energy / p.energy
        )
        p_heatnet_cogen.CO2e_cb_per_MWh = (
            fact('Fact_H_P_heatnet_biomass_ratio_CO2e_cb_to_fec_2018')
        )
        ...
        p_biomass.CO2e_pb_per_MWh = (
            fact('Fact_H_P_biomass_ratio_CO2e_pb_to_fec_2018')
        )
        p_opetpro.change_energy_MWh = (
            p_opetpro.energy - h18.p_opetpro.energy
        )
        p_opetpro.change_energy_pct = (
            p_opetpro.change_energy_MWh / h18.p_opetpro.energy
        )
        p_opetpro.change_CO2e_t = (
            p_opetpro.CO2e_total - h18.p_opetpro.CO2e_total
        )
        p_opetpro.change_CO2e_pct = (
            p_opetpro.change_CO2e_t / h18.p_opetpro.CO2e_total
        )
        p_opetpro.CO2e_total_2021_estimated = (
            h18.p_opetpro.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p.change_energy_MWh = (
            p.energy - h18.p.energy
        )
        p_heatnet.pct_energy = (
            p_heatnet.energy / p.energy
        )
        p_biomass.cost_fuel_per_MWh = (
            fact('Fact_R_S_wood_energy_cost_factor_2018')
        )
        p_coal.cost_fuel = (
            p_coal.energy *
            p_coal.cost_fuel_per_MWh / Million
        )
        p_heatnet_lheatpump.invest_pa = (
            p_heatnet_lheatpump.invest / entry('In_M_duration_target')
        )
        p_biomass.CO2e_pb = (
            0 * p_biomass.CO2e_pb_per_MWh
        )
        p_biomass.CO2e_total = p_biomass.CO2e_pb
        p_ofossil.CO2e_pb_per_MWh = (
            fact('Fact_H_P_ofossil_ratio_CO2e_pb_to_fec_2018')
        )
        p_ofossil.CO2e_pb = (
            0 * p_ofossil.CO2e_pb_per_MWh
        )
        p_coal.change_energy_MWh = (
            p_coal.energy - h18.p_coal.energy
        )
        p_coal.change_energy_pct = (
            p_coal.change_energy_MWh / h18.p_coal.energy
        )
        p_coal.change_CO2e_t = (
            p_coal.CO2e_total - h18.p_coal.CO2e_total
        )
        p_coal.change_CO2e_pct = (
            p_coal.change_CO2e_t / h18.p_coal.CO2e_total
        )
        p_coal.CO2e_total_2021_estimated = (
            h18.p_coal.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_biomass.pct_energy = (
            p_biomass.energy / p.energy
        )
        p_ofossil.CO2e_total = p_ofossil.CO2e_pb

        p_heatnet.CO2e_cb = p_heatnet_cogen.CO2e_cb

        p_orenew.CO2e_pb_per_MWh = (
            fact('Fact_H_P_orenew_ratio_CO2e_pb_to_fec_2018')
        )
        p_heatnet.change_energy_MWh = (
            p_heatnet.energy - h18.p_heatnet.energy
        )
        p_heatnet.change_energy_pct = (
            p_heatnet.change_energy_MWh / h18.p_heatnet.energy
        )
        p_heatnet.change_CO2e_t = (
            p_heatnet.CO2e_total - h18.p_heatnet.CO2e_total
        )
        p_heatnet.change_CO2e_pct = (
            p_heatnet.change_CO2e_t / h18.p_heatnet.CO2e_total
        )
        p_heatnet.CO2e_total_2021_estimated = (
            h18.p_heatnet.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_heatnet_geoth.power_to_be_installed = (
        ...
        p_heatnet_geoth.pct_of_wage = (
            fact('Fact_B_P_constr_main_revenue_pct_of_wage_2017')
        )
        p_heatnet_geoth.invest_pa = (
            p_heatnet_geoth.invest / entry('In_M_duration_target')
        )
        p_heatnet_plant.pct_of_wage = (
            fact('Fact_B_P_constr_main_revenue_pct_of_wage_2017')
        )
        ...
        p_orenew.CO2e_pb = (
            0 * p_orenew.CO2e_pb_per_MWh
        )
        p_heatnet_cogen.pct_energy = (
            p_heatnet_cogen.energy / p_heatnet.energy
        )
        p_heatnet_plant.CO2e_pb_per_MWh = (
            fact('Fact_H_P_orenew_ratio_CO2e_pb_to_fec_2018')
        )
        ...
        p_heatnet_cogen.change_energy_MWh = (
            p_heatnet_cogen.energy -
            h18.p_heatnet_cogen.energy
        )
        p_heatnet_cogen.change_energy_pct = (
            p_heatnet_cogen.change_energy_MWh / h18.p_heatnet_cogen.energy
        )
        p_heatnet_cogen.change_CO2e_t = (
            p_heatnet_cogen.CO2e_total -
            h18.p_heatnet_cogen.CO2e_total
        )
        p_heatnet_cogen.change_CO2e_pct = (
            p_heatnet_cogen.change_CO2e_t / h18.p_heatnet_cogen.CO2e_total
        )
        p_heatnet_cogen.CO2e_total_2021_estimated = (
            h18.p_heatnet_cogen.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_heatnet_geoth.ratio_wage_to_emplo = (
            fact('Fact_B_P_constr_main_ratio_wage_to_emplo_2017')
        )
        ...
        p_heatnet_lheatpump.demand_electricity = (
            p_heatnet_lheatpump.energy / fact('Fact_H_P_heatnet_lheatpump_apf')
        )
        p.demand_electricity = p_heatnet_lheatpump.demand_electricity
        p_heatnet_geoth.CO2e_pb_per_MWh = (
            fact('Fact_H_P_orenew_ratio_CO2e_pb_to_fec_2018')
        )
        ...
        p_heatnet_lheatpump.demand_emplo_com = (
            p_heatnet_lheatpump.demand_emplo
        )
        ...
        p_heatnet_geoth.CO2e_total_2021_estimated = (
            h18.p_heatnet_geoth.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        h.invest_com = (
            g.invest_com + p.invest_com
        )
        h.invest = (
            g.invest + p.invest
        )
        p.change_energy_pct = (
            p.change_energy_MWh / h18.p.energy
        )
        p_fueloil.pct_energy = (
            p_fueloil.energy / p.energy
        )
        p_ofossil.pct_energy = (
            p_ofossil.energy / p.energy
        )
        p_biomass.cost_fuel = (
            p_biomass.energy *
            p_biomass.cost_fuel_per_MWh / Million
        )
        p.cost_fuel = (
            p_gas.cost_fuel +
            p_fueloil.cost_fuel +
        ...
        p.change_CO2e_pct = (
            p.change_CO2e_t / h18.p.CO2e_total
        )
        p_biomass.change_energy_MWh = (
            p_biomass.energy -
            h18.p_biomass.energy
        )
        p_biomass.change_energy_pct = (
            p_biomass.change_energy_MWh / h18.p_biomass.energy
        )
        p_biomass.change_CO2e_t = (
            p_biomass.CO2e_total -
            h18.p_biomass.CO2e_total
        )
        p_biomass.change_CO2e_pct = 0

        p_biomass.CO2e_total_2021_estimated = (
            h18.p_biomass.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_ofossil.change_energy_MWh = (
            p_ofossil.energy -
            h18.p_ofossil.energy
        )
        p_ofossil.change_energy_pct = (
            p_ofossil.change_energy_MWh / h18.p_ofossil.energy
        )
        p_ofossil.change_CO2e_t = (
            p_ofossil.CO2e_total -
            h18.p_ofossil.CO2e_total
        )

        p_ofossil.change_CO2e_pct = 0
        p_ofossil.CO2e_total_2021_estimated = (
            h18.p_ofossil.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_orenew.change_energy_MWh = (
            p_orenew.energy - h18.p_orenew.energy
        )
        p_orenew.change_energy_pct = (
            p_orenew.change_energy_MWh / h18.p_orenew.energy
        )
        p_orenew.change_CO2e_t = (
            p_orenew.CO2e_total - h18.p_orenew.CO2e_total
        )
        p_orenew.change_CO2e_pct = 0

        p_orenew.CO2e_total_2021_estimated = (
            h18.p_orenew.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_solarth.pct_energy = (
            p_solarth.energy / p_orenew.energy
        )
        p_solarth.CO2e_pb_per_MWh = (
            fact('Fact_H_P_orenew_ratio_CO2e_pb_to_fec_2018')
        )
        p_solarth.CO2e_pb = (
            0 * p_solarth.CO2e_pb_per_MWh
        )
        p_solarth.CO2e_total = p_solarth.CO2e_pb
        p_solarth.change_energy_MWh = (
            p_solarth.energy - h18.p_solarth.energy
        )
        p_solarth.change_energy_pct = (
            p_solarth.change_energy_MWh / h18.p_solarth.energy
        )
        p_solarth.change_CO2e_t = (
            p_solarth.CO2e_total - h18.p_solarth.CO2e_total
        )
        p_solarth.change_CO2e_pct = 0
        p_solarth.CO2e_total_2021_estimated = (
            h18.p_solarth.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
        ...
        p_heatpump.pct_energy = (
            p_heatpump.energy / p_orenew.energy
        )
        p_heatpump.CO2e_pb_per_MWh = (
            fact('Fact_H_P_orenew_ratio_CO2e_pb_to_fec_2018')
        )
        p_heatpump.CO2e_pb = (
            0 * p_heatpump.CO2e_pb_per_MWh
        )
        p_heatpump.CO2e_total = p_heatpump.CO2e_pb

        p_heatpump.change_energy_MWh = (
            p_heatpump.energy - h18.p_heatpump.energy
        )
        p_heatpump.change_energy_pct = (
            p_heatpump.change_energy_MWh / h18.p_heatpump.energy
        )
        p_heatpump.change_CO2e_t = (
            p_heatpump.CO2e_total -
            h18.p_heatpump.CO2e_total
        )
        p_heatpump.change_CO2e_pct = 0

        p_heatpump.CO2e_total_2021_estimated = (
            h18.p_heatpump.CO2e_total *
            fact('Fact_M_CO2e_wo_lulucf_2021_vs_2018')
        )
