...
def lookUpAGSinRepo(ags:str,lookUpDict,repo) -> list[str]:
    ...
    if len(vorkommnisse) == 1:
        [_,_,name,validFrom,changeType,validUntil,successorAGS,successorName,successorAGSValidFrom,descritpion] = dataList[vorkommnisse[0][0]]
...
def main ():
    ...
    BiomassDict = handleNotInMaster(BiomassDict,master=masterAGS,dataList=agsList,lookUpDict=lookUpDict)
    PvDict = handleNotInMaster(PvDict,master=masterAGS,dataList=agsList,lookUpDict=lookUpDict)
    WasserDict = handleNotInMaster(WasserDict,master=masterAGS,dataList=agsList,lookUpDict=lookUpDict)
    WindDict =  handleNotInMaster(WindDict,master=masterAGS,dataList=agsList,lookUpDict=lookUpDict)
